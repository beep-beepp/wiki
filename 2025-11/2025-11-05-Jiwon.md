...
- Using store barriers
  ...
  - Young/Old 간 참조 변화를 추적하기 위해 사용한다.
    - JDK 24부터 ZGC는 무조건 Generational 모드이므로 store barrier는 항상 활성화되어 있다.
...
## 특징
ZGC는 모든 비용이 큰 작업을 Concurrent로 수행하며, 애플리케이션 스레드의 pause time을 1ms 미만으로 유지한다.
...
2. Concurrent Mark/Remap
   ...
   - Colored pointer를 이용해 load barrier가 remapping 작업을 지원한다.
3. Pause Mark End 
...
5. Pause Relocate Start
   - root가 relocation set을 가리키는 참조를 처리
   - relocation 과정 중 루트 참조가 유효성을 잃지 않도록 root forwarding 테이블을 업데이트한다.
   - 매우 짧은 pause
...
<br>finalizer는 Java 9부터 deprecated 되었기 때문에, Generational ZGC에서도 Finalizable bit는 과거 호환성 용도로만 존재한다.
...
- [finalize](https://docs.oracle.com/javase/8/docs/api/java/lang/Object.html#finalize%E2%80%93)
- [finalize deprecated](https://docs.oracle.com/en/java/javase/18/docs/api/java.base/java/lang/Object.html#finalize())
