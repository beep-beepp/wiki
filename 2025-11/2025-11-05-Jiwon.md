...
4. Pause Young Relocate Start
   - Concurrent Young Mark 중 Young generation의 region을 분석하여 live ratio가 낮은 region은 relocation set으로 선택한다. 
   - Young roots가 relocation set을 가리키는 참조를 처리, root forwarding 테이블을 업데이트한다(old 참조 무효화 방지).
   - 매우 짧은 pause
...
4. Pause Relocate Start
   - Concurrent Mark 중 low live ratio regions은 relocation set으로 선택한다.
